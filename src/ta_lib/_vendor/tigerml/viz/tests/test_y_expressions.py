...
def test_initial_values(test_df):
    ...
    assert all(explorer.y_exprs.children[0].sort_rule.values) == all(
        ["", "ASC", "DESC"]
    )
...
)  # ("bool_col", False) has to be debugged
@settings(max_examples=10, deadline=None)
@given(test_df=mixed_df())
def test_options(test_df, col_name, num_col):
    ...
    if num_col:
        assert all(explorer.y_exprs.children[0].agg_func.options) == all(
            ["", "sum", "mean", "max", "min", "count", "distinct count"]
        )
        assert all(explorer.color_axis.options) == all(expected_segmentby_options)
        assert all(explorer.y_exprs.children[0].plot_type.options) == all(
            ["kde", "hist", "box", "violin", "table"]
        )
    else:
        assert all(explorer.y_exprs.children[0].agg_func.options) == all(
