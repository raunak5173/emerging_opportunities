...
class VizWidget(WidgetUI):
    ...
    def __init__(
        ...
        initial_state={},
        children=None,
        data_access=True,  # noqa
        show_summary=False,
        debugger=False,
        datashade_beyond=None,
    ):
        """
        ...
        self.DATASHADE_BEYOND = (
            datashade_beyond or 200000
        )  # Beyond these number of points, use datashader
        # if data is not None:
        #     self._initiate()
    ...
    def _compute_dtypes(self):
        ...
        if self.processed_data is not None:
            ...
            remaining_cols = [
                x
                for x in self.processed_data.columns
                if x not in num_cols + bool_cols + dt_cols + cat_cols
            ]  # noqa
    ...
    def save_data(self, event=None):
        ...
        x_col = (
            self.current_state["x_col.value"]
            if "x_col.value" in self.current_state
            else self.initial_state["x_col.value"]
        )  # noqa
        group_by_cols = (
            self.current_state["splitter.value"]
            if "splitter.value" in self.current_state
            else self.initial_state["splitter.value"]
        )  # noqa
    ...
    def preprocess_data(self, recompute=False):
        ...
        if (
            not self.data_processor.has_state_change()
            and not self.data_loader.has_state_change()
            and not recompute
        ):
            return
    ...
    def filter_data(self, recompute=False):
        ...
        # pdb.set_trace()
        if (
            not self.filters.has_changes
            and not self.data_processor.has_state_change()
            and not self.data_loader.has_state_change()
            and not recompute
        ):
            return
    ...
    def sort_data(self, recompute=False):
        ...
        if self.has_data_changes or self.has_sort_changes or recompute:
            self.y_exprs.sort_data(
                self.x_col.value, self.x_sort.value, group_by_cols=self.splitter.value
            )
    ...
    def create_plot_data(self, data=None, recompute=False):
        ...
        self.y_exprs.compute_plot_data(
            data=data,
            x_col=self.x_col.value,
            split_by_cols=self.splitter.value,
    ...
    def open(self, port=None, threaded=True):
        ...
        if port:
            self.server = self.pane.show(
                title="TigerML Explorer", port=port, threaded=threaded
            )
        else:
    ...
    def widget_title(self):
        ...
        return ", ".join([y.description for y in self.y_exprs.children]) + (
            " vs {}".format(self.x_col.value) if self.x_col.value != "" else ""
        )
