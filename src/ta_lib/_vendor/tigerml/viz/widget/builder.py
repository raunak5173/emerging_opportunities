...
class WidgetBuilder(VizWidget):
    ...
    def show_preview(self, event=None):
        ...
        preview = WidgetView(
            ...
            free_states,
            data=self.data,
            delete_callback=self.delete_preview,
        )  # noqa
        preview.create_pane()
    ...
    def modify_child_creation(self, obj, prefix="lock"):
        ...
        def new_func(*args, **kwargs):
            ...
            self.make_states_lockable(
                child, child.get_state(), prefix=f"{prefix}children_{uid}"
            )
            self.lock_widget[0] = ""
    ...
    def make_states_lockable(self, element, element_states, prefix="lock"):
        ...
        if "children" in element_states:
            for ind, child_state in enumerate(element_states["children"]):
                self.make_states_lockable(
                    element.children[ind], child_state, prefix=f"{prefix}children_{ind}"
                )
    ...
    def open(self, port=None):
        ...
        self.lock_widget = pn.Row(
            self.lockable_elements,
            self.unlock_element_btn,
            self.lock_element_btn,
        ...
        self.tmpl.add_panel(
            "lock_selector",
            self.Column(
                self.lock_widget, self.Row(self.show_preview_btn, self.save_state_btn)
            ),
        )  # noqa
