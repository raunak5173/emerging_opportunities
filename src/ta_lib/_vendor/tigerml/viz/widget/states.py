...
class StateMixin:
        ...
        for key in state:
            if "value" in key:
                if hasattr(self, key.replace(".value", "")):
                    ...
                    _LOGGER.warning(
                        f'{self.__class__.__name__} does not have "{key.replace(".value", "")}" attribute. Omitting.'
                    )  # noqa
    ...
    def get_ui_state(self, include_children=True):
        ...
        if include_children:
            state.update(
                {"children": [child.get_ui_state() for child in self.children]}
            )
    ...
    def has_state_change(self, *args, include_children=True):
        ...
        if args:
            ...
            changes = [
                self.current_state[arg] != self._get_property(arg) for arg in args
            ]
        else:
            changes = [
                self.current_state[arg] != self._get_property(arg)
                for arg in self.current_state
            ]
            if include_children:
                changes += [child.has_state_change() for child in self.children] + [
                    self.current_state["no_of_children"] != len(self.children)
                ]
        return any(changes)
    ...
    def _check_state_for_data(self, data):
        return all(
            [
                self.current_state[f"{key}.value"] in data.columns
                for key in self.data_dependent
            ]
        ) and all(
            [child.check_state_for_data(data) for child in self.children]
        )  # noqa
    ...
    def _update_data_columns(self, data=None, dtypes=None):
        ...
        if self.data is not None:
            for key in self.data_dependent:
                exec(
                    f"self.{key}.options = self.data_dependent[key](self.data, self.dtypes)"
                )
            for child in self.children:
...
class StatefulUI(StateMixin, PanelBackend):
    ...
    def __init__(self, children=None, dynamic_ui=False, initial_state={}):
        super().__init__(
            children=children, dynamic_ui=dynamic_ui, initial_state=initial_state
        )

    def _create_ui_from_config(self, ui_config, data=None, dtypes=None):
        ui_dict, data_dependent, stateful_list = self.create_ui_from_config(
            ui_config, data, dtypes, initial_state=self.initial_state
        )  # noqa
        for key in ui_dict:
