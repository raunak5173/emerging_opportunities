...
class WidgetView(VizWidget):
    ...
    def __init__(
        self, initial_state, free_states, *args, delete_callback=None, **kwargs
    ):  # noqa
        ...
        if delete_callback:
            ...
            self.close_btn = self.Button(
                css_clsses=["icon-button", "icon-cancel", "right"], width=50, height=50
            )
    ...
    def describe(self, options, row=False):
        ...
        if isinstance(options, list):
            ...
            x_description = ""
            if (
                "x_col.value" in self.initial_state
                and self.initial_state["x_col.value"]
            ):
                x_description = self.Row(
                    f'<b> vs </b>{self.initial_state["x_col.value"]}',
                    css_classes=["description"],
                )  # noqa
            elif "x_col.value" in self.free_states:
                x_description = self.Row(
                    " vs ", self.x_col, css_classes=["description"]
                )
            ...
            if (
                ...
                and self.initial_state["splitter.value"]
            ):
                splitter_description = self.Row(
                    f' split by {self.initial_state["splitter.value"]}',
                    css_classes=["description"],
                )  # noqa
            elif "splitter.value" in self.free_states:
                splitter_description = self.Row(
                    " split by ", self.splitter, css_classes=["description"]
                )
            if splitter_description and "split_plots.value" in self.free_states:
                self.split_plots.name = "Show in a Grid"
                self.split_plots.css_classes = self.split_plots.css_classes + [
                    "show_grid_option"
                ]
            ...
            raise Exception(
                "options should be one or multiple of x, y, filters, splitter"
            )
