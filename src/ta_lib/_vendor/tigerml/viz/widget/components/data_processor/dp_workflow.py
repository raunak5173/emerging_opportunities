...
class DPWorkFlow(StatefulUI):
    ...
    def __init__(self, initial_data, parent_widget=None, name=None, parent=None):
        ...
        if not self.parent:
            self.pane.insert(
                0, self.SectionHeader('<h3 class="section_header">Preprocessing: </h3>')
            )
        self.main_pane = self.Column(
            self.pane, self.overlays, css_classes=["dp_workflow"]
        )
    ...
    def create_dp_workflow(self):
        ...
        self.dp_workflow = self.Row(
            *([child.create_pane() for child in self.children] + [self.start_dp]),
            css_classes=["dp_linear_flow"]
        )
        if self.parent is None:
            self.dp_workflow.insert(
                0,
                self.source_node
                if hasattr(self, "source_node")
                else self.get_source_node(),
            )
    ...
    def get_source_node(self):
        ...
        if not hasattr(self, "source_node"):
            ...
            self.source_block_ui = self.Column(
                self.Row(view_source_btn, css_classes=["buttons"]),
                self.HTML("", css_classes=["node"]),
                self.Row("Source Data", css_classes=["op_summary"]),
                css_classes=["block_ui"],
            )
            self.source_node = self.Row(
                self.source_block_ui,
                self.add_button,
                css_classes=["operation_block", "is_active"],
                width=150,
            )
    ...
    def check_error_in_workflow(self, from_dp_index):
        ...
        if self.error_in_workflow["node"] != -2:
            error_msg = "Error in Node-{}: {}".format(
                self.error_in_workflow["node"] + 1, self.error_in_workflow["error"]
            )
