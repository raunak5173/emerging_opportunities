...
class Transformer(StatefulUI):
    ...
    def create_ui(self):
        ...
        self.pane.extend(
            [
                self.transform_type,
                self.Row(self.new_col_name, self.col_selection_block),  # noqa
                self.op_info_block,
                self.code_input,
            ]
        )
    ...
    def _initiate(self):
        self.transform_type = self.Select(
            name="Transformation type",
            options=[
                "Column-wise transform",
                "Groupby transform",
                "Groupby aggregated transform",
                "Row-wise transform",
            ],
            width=200,
        )
        self.transform_type.param.watch(self.opt_func, "value")
        self.new_col_name = self.TextInput(
            name="New column name",
            placeholder="Enter the column name to be added...",
            width=200,
        )
        self.select_columns = self.MultiChoice(
            name="Select the cols to trans", options=self.input_data_cols, width=200
        )
        self.select_grpby_cols = self.MultiChoice(
            name="Select the cols to groupby", options=self.input_data_cols, width=200
        )
        self.select_column = self.Select(
            name="Select the col to trans", options=self.input_data_cols, width=200
        )
        ...
        self.code_input = self.CodeInput(
            name="pre process code",
            value=code_snippet,
            height=400,
            width=500,
            language="python",
    ...
    def opt_func(self, event=None):
        ...
        if event.new == "Column-wise transform":
            op_info = (
                "Note: Keep the main function name as 'func', "
    ...
    def exec_func(self, data):
        ...
        if self.transform_type.value == "Column-wise transform":
            ...
            """
            processed_data[self.new_col_name.value] = processed_data[
                [self.select_column.value]
            ].apply(
                lambda x: transform_code.func(x)
            )  # noqa
            ...
            wrapper_func_, transform_func = generate_func_strings(
...
class CustomTransformer(StatefulUI):
    ...
    def __init__(self, data):
        super().__init__()
        self.name = "Custom"
        self.operation_name = self.TextInput(
            name="Transformation name (optional)",
    ...
    def opt_state(self):
        """A method that returns the state of the Operation."""
        return {
            "operation_name.value": self.operation_name.value,
            "code_input.value": self.code_input.value,
        }
