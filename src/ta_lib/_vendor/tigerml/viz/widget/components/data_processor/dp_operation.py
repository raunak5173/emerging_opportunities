...
class DPOperation(StatefulUI):
    ...
    def __init__(self, operator, parent=None):
        ...
        self.active = self.Toggle(
            width=24, height=24, value=True, css_classes=["icon-button", "icon-ok"]
        )  # noqa
        self.active.param.watch(self.toggle_active, "value")
        self.current_state.update(
            {"active.value": self.active.value, "operator.name": self.operator.name}
        )
        self.view_button = self.Button(
            width=24, height=24, css_classes=["icon-button", "icon-view"]
        )
        ...
        self.edit_button = self.Button(
            width=24, height=24, css_classes=["icon-button", "icon-edit"]
        )
        ...
        self.add_button = self.Button(
            name="+", css_classes=["add_button"], width=30, height=30
        )
        ...
        self.delete_button = self.Button(
            name="Delete Operation", width=130, css_classes=["button", "tertiary"]
        )
        self.delete_button.on_click(self.delete_operation_node)
        self.delete_button.js_on_click(args={}, code=self.remove_glass)
        self.save_edited_button = self.Button(
            name="Save Operation", width=130, css_classes=["button", "primary"]
        )
        ...
        self.buttons = self.Row(
            self.active, self.view_button, self.edit_button, css_classes=["buttons"]
        )
        self.inactive_flag = self.HTML(
            "", css_classes=["is_inactive"], height=0, width=0
        )
    ...
    def create_pane(self):
        ...
        self.error = False
        self.summary_ui = self.Row(
            self.get_operation_summary(), css_classes=["op_summary"]
        )
        self.block_ui = self.Column(
            self.buttons,
        ...
        self.pane = self.Row(
            self.block_ui, self.add_button, css_classes=["operation_block"], width=150
        )
        # if self.error:
    ...
    def view_output_data(self, event=None):
        ...
        try:
            ...
            row = self.Overlay_inplace(
                self.HTML(f"<pre> {self.parent.error_in_workflow['traceback']} </pre>")
            )
    ...
    def input_data(self):
        ...
        if step_index > 0:
            prev_active_steps = [
                child
                for child in self.parent.children[:step_index]
    ...
    def get_operation_summary(self):
        ...
        if type == "Sort":
            summary += " in {} order".format(
                ", ".join(
                    [
                        "ascending"
                        if child["sort_type.value"] == "ASC"
                        else "descending"
                        for child in opt_dict["children"]
                    ]
                )
            )
        elif type == "Bin":
    ...
    def delete_operation_node(self, event=None):
        ...
        if self in self.parent.branches.values():
            branches = [
                branch
                for branch in self.parent.branches
                if self.parent.branches[branch] == self
            ]
