...
class DataProcessing(PanelBackend):
    ...
    def __init__(self, data, operation=None, parent=None):
        ...
        self.operations = self.Select(
            name="Select operation",
            options=[""] + list(self.DP_OPTIONS.values()),
            width=200,
        )  # noqa
        ...
        self.update_button = self.Button(
            name="View Output", width=130, css_classes=["button", "secondary"]
        )
        ...
        self.buttons = self.Row(
            self.save_button, self.update_button, self.delete_operation
        )
        ...
        self.pane = self.Row(
            self.Column(self.display_df(self.data)),
            self.Column(
                "Operations Panel",
                self.operations,
                self.operations_wrapper,
                self.buttons,
            ),
            self.output_data_preview,
        )
        self.main_pane = self.Column(
            self.display_df(self.data), css_classes=["widget_header"]
        )
    ...
    def display_df(self, data, table_width=600):
        ...
        if isinstance(data, str):
            return self.Column(
                data, height=table_height + 110, width=600, background="#ffffff"
            )
        else:
            PREVIEW_LIMIT = 1000
            ...
            return self.Column(
                self.Row(
                    hvPlot(data).table(
                        width=(len(data.columns) + 1) * 100, height=table_height
                    ),
                ...
                self.Row("shape of data = {}".format(data.shape), height=30),
                background="#ffffff",
            )
    ...
    def save_operation(self, event=None):
        ...
        if "DataFrame" in str(type(self.processed_data)):
            ...
            if self.parent is not None:
                self.parent.complete_active_operation(
                    self.operation, self.processed_data
                )
        else:
            self.output_data_preview[0] = self.display_df(
                "Error in executing operation!"
            )
    ...
    def opt_func(self, event=None):
        ...
        if self.operation is not None:
            if not event.new:  # If the user did not seletc any option
                ...
                selection_option = list(self.DP_OPTIONS.keys())[
                    list(self.DP_OPTIONS.values()).index(event.new)
                ]
                if (
                    selection_option == self.operation.name
                ):  # If the option user selected is same as the operation already present  # noqa
                    return
        self.processed_data = ""
