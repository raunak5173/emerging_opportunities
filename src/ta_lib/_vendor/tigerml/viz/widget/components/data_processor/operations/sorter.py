...
class Sorter(StatefulUI):
    ...
    def __init__(self, parent, columns):
        ...
        self.select_col = self.Select(
            name="Select col to sort by", options=self.process_columns, width=200
        )  # noqa
        self.sort_type = self.ToggleGroup(
            options=["ASC", "DESC"],
            name="Sort type",
            width=100,
            behavior="radio",
        ...
        self.del_button = PanelBackend.Button(
            width=40, height=36, css_classes=["icon-button", "icon-delete"]
        )
        self.del_button.on_click(self.delete_level)
        self.current_state.update(
            {
                "select_col.value": self.select_col.value,
                "sort_type.value": self.sort_type.value,
            }
        )
...
class Sorters(StatefulUI):
    ...
    def __init__(self, data):
        ...
        self.add_level = self.Button(
            name="Add level", width=60, css_classes=["button", "primary"]
        )
    ...
    def create_ui(self):
        ...
        self.pane.extend(
            [
                self.add_level,
                self.Column(*[child.get_pane() for child in self.children]),
            ]
        )
        return self.pane
    ...
    def exec_func(self, data):
        ...
        child_states = [
            [child[x] for x in child if ".value" in x] for child in children_state
        ]
