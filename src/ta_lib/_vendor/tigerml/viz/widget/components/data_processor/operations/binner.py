...
class Binner(StatefulUI):
    ...
    def __init__(self, data, parent=None):
        ...
        self.new_col_name = self.TextInput(
            ...
            placeholder="Enter the binned column name...",  # noqa
            width=200,
        )
        self.bin_col = self.Select(
            name="Select the cols to bin", options=self.input_data_cols, width=200
        )
        self.bin_nos = self.Select(
            name="No of bins", options=list(range(3, 11)), width=50
        )
        self.current_state.update(
            {"bin_col.value": self.bin_col.value, "bin_nos.value": self.bin_nos.value}
        )
    ...
    def exec_func(self, data):
        ...
        if self.bin_col.value:
            ...
            new_col_name = (
                self.new_col_name.value
                if self.new_col_name.value
                else "binned_" + self.bin_col.value
            )
            processed_data[new_col_name] = pd.cut(
                processed_data[self.bin_col.value], bins=self.bin_nos.value
            )
            ...
            processed_data[new_col_name] = (
                processed_data[new_col_name].astype(str).astype(cat_type)
            )
