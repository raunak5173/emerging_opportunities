...
class YExprUI(StatefulUI):
    ...
    def _compute_y_cols(self, data, dtypes):
        ...
        return get_num_cols(data) + [
            x for x in data.columns if x not in get_num_cols(data)
        ]  # noqa

    def _create_ui(self):
        col_options_func = lambda data, dtypes: self._compute_y_cols(data, dtypes)
        col_name = self.select(
            width=210, name="Select Column(Y)", options=col_options_func
        )
        from functools import partial

        agg_func = self.select(
            value="",
            name="Aggregation",
            width=110,
            options=partial(self.agg_options, None),
        )
        have_color_axis = self.checkbox(
            name="Have Color Axis", width=110, css_classes=["color_axis_checkbox"]
        )
        ...
        normalize_by = self.select(
            name="Normalize by", options=["count", "sum"], width=80
        )
        normalize_across = self.multichoice(
            name="Normalize across", options=[], width=225
        )
        axis = self.togglegroup(
            options=["left", "right"],
            name="Y Axis",
            width=100,
            behavior="radio",
        ...
        if not self.initial_state:
            if self.data is not None:
                self.agg_func.value = (
                    "count"
                    if self.col_name.value not in get_num_cols(self.data)
                    else ""
                )
            ...
            if (
                "sort_rule.value" not in self.initial_state
                and len(  # noqa
                    [
                        y
                        for y in self.parent.children
                        if "sort_rule.value" not in y.initial_state
                    ]
                )
                > 1
            ):
    ...
    def _set_defaults(self):
        ...
        self.current_state["plot_type.value"] = (
            self.plot_type.options[0] if self.plot_type.options else ""
        )
    ...
    def activate_normalizer(self, event=None):
        ...
        if (
            self.agg_func.value in ["sum", "count"]
            and self.normalize_across.value != []
        ):
    ...
    def create_pane(self):
        ...
        if not self.ui_created:
            self._create_ui()
        self.y_def = self.Row(
            self.agg_func, self.col_name, width=460, css_classes=["y_def"]
        )
        self.layout_options = self.Row(
            self.plot_type,
        ...
        self.advanced_options = self.Row(
            self.normalize,
            self.sort_rule,
            width=460,
            css_classes=["y_advanced_options"],
        )
        self.pane = self.Column(
            self.y_def, self.layout_options, width=460, css_classes=["y_expr"]
        )
