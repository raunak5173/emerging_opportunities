...
class YExpr(YExprUI):
    ...
    def display_name(self):
        ...
        return (
            "{}({})".format(self.agg_func.value, self.y_col)
    ...
    def description(self):
        ...
        return (
            self.display_name + " segmented by {}".format(self.segment_by)
            if self.segment_by
    ...
    def has_multiple_y_values(self, ignore_segment=True):
        ...
        if self.parent.parent.splitter.value:
            ...
            if index_name:
                ...
                value = any(
                    self.plot_data.groupby(groupby_cols)
                    .apply(lambda df: df[index_name].nunique() < len(df))
                    .values
                )  # noqa
                ...
                value = any(
                    self.plot_data.groupby(groupby_cols)
                    .apply(lambda df: df.index.nunique() < len(df))
                    .values
                )
        else:
            if not ignore_segment and self.segment_by:
                ...
                if index_name:
                    self.plot_data.reset_index(inplace=True)
                    value = any(
                        self.plot_data.groupby(self.segment_by)
                        .apply(lambda df: df[index_name].nunique() < len(df))
                        .values
                    )  # noqa
                    self.plot_data.set_index(index_name, inplace=True)
    ...
    def _is_valid_state(self, event):
        if event and event.obj.name in [
            "Select Columns(Splitter)",
            "Select Column(X)",
            "Select Column(Y)",  # noqa
            "Color Axis",
            "Have Color Axis",
            "UPDATE PLOT",
            "Aggregation",
            ...
            for key in condition_dict.keys():
                if eval(key):
                    ...
                    return False
            if (
                event and event.obj.name != "UPDATE PLOT"
            ):  # In order to show the loading message
    ...
    def refresh_plot_types(self, event=None):
        ...
        if hasattr(self.parent.parent, "x_col") and self._is_valid_state(event):
            ...
            self.x_type = (
                ...
                if self.dtypes[x_col] in ["numeric", "datetime"]
                else "non_numeric"
            )  # noqa
            if self.agg_func.value:
                self.y_type = "numeric"
                if self.x_type == "no_x":
                    self.x_type = (
                        "numeric"
                        if self.dtypes[self.y_col] in ["numeric", "datetime"]
                        else "non_numeric"
                    )
            ...
            if len(self.data.columns) != len(
                self.parent.parent.filtered_data.columns
            ) and set(list(self.data.columns)) != set(
                list(self.parent.parent.filtered_data.columns)
            ):
                self.parent.parent.filter_data()
            self.compute_plot_data(
                self.parent.parent.filtered_data,
                x_col,
                self.parent.parent.splitter.value.copy(),
            )
            self.no_of_y_values = (
                "multiple_y"
                if self.has_multiple_y_values(ignore_segment=False)
                else "single_y"
            )
            logging.info(f"X col is {x_col} - {self.x_type}")
            logging.info(
                f"y col is {self.y_col} - {self.y_type} - {self.no_of_y_values}"
            )
            if self.x_type == "no_x":
                self.plot_types = PLOT_TYPES[self.x_type][self.y_type][
                    self.has_color_axis
                ].copy()
            else:
                self.plot_types = PLOT_TYPES[self.x_type][self.y_type][
            ...
            if self.plot_type.options != new_options:
                ...
                self.plot_type.value = self.plot_type.options[
                    0
                ]  # noqa  # to avoid robustness issue (the option seen in UI and
    ...
    def has_data_changes(self):
        ...
        return self.has_state_change(
            "col_name.value",
            "agg_func.value",
            "have_color_axis.value",
            "normalize_by.value",
            "normalize_across.value",
        )  # noqa
    ...
    def _get_plot_type_options(self):
        ...
        ptype_index = [ptype[0] for ptype in self.plot_types].index(
            self.plot_type.value
        )  # noqa
        ptype_kwargs = self.plot_types[ptype_index][1]
        return ptype_kwargs

    def preprocess_data_for_plot(
        self, series_data, x_col, group_by_cols=[], segment_col=""
    ):  # noqa
        ...
        if (
            self.plot_type.value in ["heatmap"]
            or self.plot_type.value in ["bar", "grouped_bar"]
            and "stacked" in self._get_plot_type_options()
        ):  # noqa
            series_data.reset_index(inplace=True)
            plot_data = series_data.groupby(
                [x_col, self.display_name] + group_by_cols
            ).agg(len)
            # plot_data.name = 'count'
            series_data = plot_data.reset_index().rename(columns={0: "count"})
        elif (
            self.plot_type.value in ["bar", "grouped_bar"]
            and self.x_type == "no_x"
            and self.y_type == "non_numeric"
        ):  # noqa
        ...
        if x_col and get_dt_cols(series_data[[x_col]]):
            ...
            x_is_date = True
        if (
            self.display_name in self.dtypes.keys()
            and get_dt_cols(series_data[[self.display_name]])
            and x_col
        ):
            series_data[self.display_name] = (
                series_data[self.display_name].astype(np.int64) / 10**6
            )
            y_is_date = True
        if (
            not x_col
            and self.display_name in self.dtypes.keys()
            and get_dt_cols(series_data[[self.display_name]])
        ):
            series_data[self.display_name] = (
                series_data[self.display_name].astype(np.int64) / 10**6
    ...
    def set_kwargs_for_bars(
        ...
        if not only_bars and segment_col and segment_col not in group_by_cols:
            if multiple_series:
                ...
                series_data[segment_col + "_levels"] = prefix + series_data[
                    segment_col
                ].astype(str)
                ...
                if self.dtypes[segment_col] not in ["numeric", "datetime"]:
                    series_data[segment_col + "_levels"] = series_data[
                        segment_col
                    ].astype(str)
                    # kwargs["by"] = segment_col + "_levels"
                    kwargs["by"] = segment_col
    ...
    def post_process_plot(
        ...
        if "datashade" in kwargs and kwargs["datashade"]:
            if mapper is not None:
                ...
                ticks = list(
                    zip(
                        mapper_df["tigerml_mapper"].values.tolist(),
                        get_formatted_values(mapper_df[x_name]),
                    )
                )
    ...
    def set_kwargs_for_plotter(
        self, plotter, kwargs, series_data, x_col, y_col, segment_col, group_by_cols
    ):
        ...
        if group_by_cols:
            ...
            if index_name:
                series_data.reset_index(inplace=True)
                max_data_length = any(
                    series_data.groupby(group_by_cols).apply(
                        lambda df: len(df) > self.parent.parent.DATASHADE_BEYOND
                    )
                )  # noqa
                series_data.set_index(index_name, inplace=True)
            else:
        ...
        if len(series_data) > self.parent.parent.DATASHADE_BEYOND and max_data_length:
            if kwargs["kind"] in ["scatter", "line"]:
                ...
                if x_name is not None:
                    unique_vals = (
                        new_data[x_name].unique().categories.tolist()
                        if str(new_data.dtypes[x_name]) == "category"
                        else list(new_data[x_name].unique())
                    )  # noqa
                    mapper = dict(
                        zip(unique_vals, range(0, new_data[x_name].nunique()))
                    )
                    mapper_df = (
                        td.DataFrame(mapper, index=[0])
                        .T.reset_index()
                        .rename(columns={0: "tigerml_mapper", "index": x_name})
                    )  # noqa
        ...
        if "by" in kwargs:
            if kwargs["kind"] == "bar":
                if (
                    self.plot_type.value == "bar"
                ):  # should not be set to True for 'grouped_bar' case
                    kwargs["stacked"] = True
            elif (
                kwargs["kind"] == "scatter"
                and kwargs["by"] not in get_cat_cols(series_data)
                and ("datashade" not in kwargs or not kwargs["datashade"])
            ):  # noqa
        ...
        if self.plot_type.value == "grouped_bar" and "by" in kwargs:
            if "invert" in kwargs and kwargs["invert"]:
                color_hook_kwargs["bar_with_color"] = (
                    kwargs["by"],
    ...
    def get_plot_by_type(
        ...
        if self.display_name in series_data.columns:
            ...
            cols_with_name = [
                col for col in series_data.columns if self.display_name in col
            ]
            if cols_with_name:
        ...
        series_data, x_is_date, y_is_date = self.preprocess_data_for_plot(
            series_data, x_col, group_by_cols, segment_col
        )  # noqa
        self.set_kwargs_for_bars(
            kwargs,
            series_data,
            x_col,
            y_col,
        ...
        (
            ...
            x_name,
            color_hook_kwargs,
        ) = self.set_kwargs_for_plotter(
            plotter, kwargs, series_data, x_col, y_col, segment_col, group_by_cols
        )
        self.plotter_kwargs = kwargs
        current_plot = plotter(**kwargs)
        entire_plot = current_plot
        if (
            current_plot.__module__ == "panel.layout"
        ):  # If the plot is split, it will be a panel layout on which the
        ...
        if "bar_with_color" in color_hook_kwargs.keys():
            ...
            color_hook_kwargs["bar_with_color"] = (
                color_hook_kwargs["bar_with_color"][1],
                color_hook_kwargs["bar_with_color"][2],
            )  # noqa
        if (
            "box_with_color" in color_hook_kwargs.keys()
        ):  # Factors of Level_1 are to be passed to colorize_boxplot hook
            level_1_factors = [
                str(x)
                for x in series_data[color_hook_kwargs["box_with_color"]]
                .dropna()
                .unique()
                .tolist()
            ]  # noqa
            color_hook_kwargs["box_with_color"] = (
                color_hook_kwargs["box_with_color"],
                level_1_factors,
            )
        current_plot = self.post_process_plot(
            current_plot,
            kwargs,
            x_col,
            last_y,
    ...
    def get_sorter(self, x_col, group_by_cols=[]):
        ...
        sorter = (
            td.DataFrame(index.values)
            .rename(columns={0: index_name})
            .rename_axis("index")
        )
        if group_by_cols:
            sorter = td.concat(
                [sorter, y_vals[group_by_cols].reset_index(drop=True)], axis=1
            )
        sorter = (
            sorter.reset_index()
            .set_index([index_name] + group_by_cols)
            .rename(columns={"index": "tigerml_sort_ranking"})
        )  # noqa
        ...
        if data.empty:
            ...
            if x_col or self.x_type != "no_x":
                ...
                if agg_func:
                    if x_col:
                        self.plot_data = (
                            data[[y] + group_by_cols]
                            .rename(columns={y: series_name})
                        ...
                        if self.normalize_data:
                            ...
                            norm_across_cols = [
                                x
                                for x in group_by_cols
                                if x not in self.normalize_across.value
                            ]
                            norm_data = (
                                data[[y] + norm_across_cols]
                                .rename(columns={y: "norm_by_value"})
                                .groupby(norm_across_cols)
                                .agg(norm_by)
                            )  # noqa
                            self.plot_data = self.plot_data.merge(
                                norm_data,
                                how="left",
                                left_on=norm_across_cols,
                                right_index=True,
                            )  # noqa
                            self.plot_data[series_name] = self.plot_data[
                                [series_name, "norm_by_value"]
                            ].apply(
                                lambda x: x[series_name] / x["norm_by_value"], axis=1
                            )
                            self.plot_data.drop(["norm_by_value"], axis=1, inplace=True)
                        self.source_data = data[group_by_cols].merge(
                            self.plot_data,
                            how="left",
                            left_on=group_by_cols,
                            right_index=True,
                        )  # noqa
                        ...
                        if self.normalize_data:
                            ...
                            self.plot_data.drop(["norm_by_value"], axis=1, inplace=True)
                        self.source_data = data[[y] + group_by_cols].merge(
                            self.plot_data,
                            how="left",
                            left_on=[y] + group_by_cols,
                            right_index=True,
                        )  # noqa
                ...
                self.plot_data = data[[y] + group_by_cols].rename(
                    columns={y: series_name}
                )
    ...
    def get_summary_stats(self, group_by_cols=None):
        ...
        if segment_col and group_by_cols:
            ...
            summary_stats = summary_df.apply(
                lambda x: x.groupby(segment_col)
                .apply(lambda df: df[y_col].describe().T)
                .T
            )  # noqa
        elif group_by_cols:
            summary_stats = (
                series_data.groupby(group_by_cols).apply(lambda df: df.describe().T).T
            )
            summary_stats.columns = summary_stats.columns.swaplevel()
        elif segment_col:
            summary_stats = (
                series_data.groupby(segment_col).apply(lambda df: df.describe().T).T
            )
            summary_stats.columns = summary_stats.columns.swaplevel()
        else:
