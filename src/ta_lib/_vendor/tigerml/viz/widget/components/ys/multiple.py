...
class YExprs(YExprsUI):
    ...
    def __init__(self, parent, data, dtypes, initial_state={}):
        self.add_yexpr = self.Button(
            name="Add New Series", css_classes=["add_y", "inline_button"]
        )  # noqa
        self.data = data
        self.parent = parent
        self.dtypes = dtypes
        ...
        if initial_state:
            [
                self.create_child(state, initial=True)
                for state in initial_state["children"]
            ]
        else:
            self._create_child()
    ...
    def has_data_changes(self):
        ...
        return any([y.has_data_changes for y in self.children]) or self.current_state[
            "no_of_children"
        ] != len(
            self.children
        )  # noqa
    ...
    def describe(self):
        ...
        ys = self.Column(
            *[y.describe() for y in self.children], css_classes=["description"]
        )
        if len(ys) > 1:
    ...
    def _add_to_plot(
        ...
        if plot.__class__.__name__ == "DynamicMap" or group_by_cols:
            ...
            if (
                ...
                and hasattr(callback.operation, "__name__")
                ...
                if group_by_cols:
                    ...
                    first_plot = eval(
                        eval_str
                    )  # getting first_plot in case of multi-level split by
        ...
        if (
            only_bars
            and plot.__class__.__name__ == "Bars"
            and not (
                y_expr.x_type in ["non_numeric", "no_x"]
                and y_expr.y_type == "non_numeric"
            )
        ):  # noqa
            plot_data = plot.data
            if segment_col:
                plot_data["metric"] = (
                    plot_data["metric"] + " - " + plot_data[segment_col].astype(str)
                )
            ...
            kwargs["x"] = (
                f"tigerml_xcol_{x_col}"
                if f"tigerml_xcol_{x_col}" in complete_data
                else x_col
            )
            kwargs["y"] = "value"
            kwargs["by"] = "metric"
            ...
            if "hooks" in plot.opts.get().kwargs:
                ...
                for i in range(len(existing_hooks)):
                    if (
                        existing_hooks[i].__class__.__name__ == "partial"
                        and existing_hooks[i].func.__name__ == "colorize_barplot"
                    ):  # noqa
                        existing_hooks[i] = partial(
                            colorize_barplot, ("metric", existing_hooks[i].args[0][1])
                        )
            ...
            if (
                current_class_name in separate_plots
                or parent_class_name in separate_plots
            ):
    ...
    def create_plot(self, x_col, split_by_cols, grid_split, show_summary):
        ...
        non_bar_plots = [
            y for y in y_exprs if y.plot_type.value not in ["bar", "grouped_bar"]
        ]
        ...
        for ind, y_expr in enumerate(y_exprs):
            ...
            current_plot = y_expr.get_plot_by_type(
                plot_series,
                group_by_cols,
                x_col,
                last_y=last_y,
                multiple_series=len(y_exprs) > 1,
                only_bars=only_bars,
            )
            if show_summary:
                ...
                if summary_df is None:
                    ...
                    summary_df.merge(
                        summary_stats, how="outer", left_index=True, right_index=True
                    )
            self.plot = self._add_to_plot(
                current_plot,
                y_expr=y_expr,
                x_col=x_col,
        ...
        if "table" not in [y_expr.plot_type.value for y_expr in y_exprs]:
            try:
                ...
                self.plot.opts(
                    ...
                    xrotation=45,
                    hooks=existing_hooks + [legend_policy],
                )
            except Exception:
        ...
        if show_summary:
            ...
            if summary_df.columns.nlevels > 1:
                summary_df.columns = [
                    f"{i}|{j}" if j != "" else f"{i}" for i, j in summary_df.columns
                ]
            summary_df.index.name = "metrics"
            summary_data = summary_df.T.reset_index().rename(
                columns={"index": "Y Series"}
            )
        ...
        for y in self.children:
            ...
            if y.segment_by:
                ...
                for level in data[y.segment_by].unique():
                    filtered_data = data[data[y.segment_by] == level].drop(
                        y.segment_by, axis=1
                    )
                    if wide_data is not None:
                        if merge_cols:
                            ...
                            wide_data = wide_data.rename(
                                columns={
                                    y.display_name: "{}_{}".format(
                                        y.display_name, level
                                    )
                                }
                            )  # noqa
                        else:
                            wide_data[
                                "{}_{}".format(y.display_name, level)
                            ] = filtered_data[y.display_name]
                    else:
                        wide_data = filtered_data
                        wide_data = wide_data.rename(
                            columns={
                                y.display_name: "{}_{}".format(y.display_name, level)
                            }
                        )
                data = wide_data
            if all_data is not None:
                all_data = all_data.merge(
                    data, how="outer", left_on=merge_cols, right_on=merge_cols
                )
            else:
                all_data = data
        if all_data is not None:
