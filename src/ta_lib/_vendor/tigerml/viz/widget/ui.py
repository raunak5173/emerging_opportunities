...
class WidgetUI(StatefulUI):
    ...
    def __init__(self, initial_state={}, children=None):
        self.loading_message = self.Markdown(
            "Computing the changes. Please wait.", height=490
        )
        if initial_state:
        ...
        self.filters = FilterPanel(
            self, self.data, self.dtypes, initial_state=filter_state
        )
    ...
    def _create_ui_config(self):
        ...
        x_col = self.get_ui_dict(
            type="select",
            name="Select Column(X)",
            options=x_col_options,
            callback=self.y_exprs.refresh_plot_types,
        )
        if not self.initial_state or all(
            [
                "sort_rule.value" not in child_state
                for child_state in self.initial_state["children"][2]["children"]
            ]
        ):  # noqa
            x_sort_callback = self.remove_y_sorts
        else:
            x_sort_callback = None
        x_sort = self.get_ui_dict(
            type="select",
            options=["", "ASC", "DESC"],
            value="ASC",
            name="Sort",
            width=70,
            callback=x_sort_callback,
        )
        x_options = self.get_ui_dict(
            type="row",
            children=params_to_dict(x_col=x_col, x_sort=x_sort),
            css_classes=["x_options"],
        )
        split_plots = self.get_ui_dict(
            type="checkbox", name="Show Plots in Grid", css_classes=["grid_option"]
        )
        splitter_options = lambda data, dtypes: sorted(
            [
                col
                for col in self.dtypes
                if self.dtypes[col] in ["bool", "category", "string"]
            ]
        )
        if not self.initial_state or (
            "splitter.value" in self.initial_state
            and "split_plots.value" in self.initial_state
        ):  # noqa
            splitter_callback = self.set_grid_option
        else:
            splitter_callback = None
        splitter = self.get_ui_dict(
            type="multichoice",
            name="Select Columns(Splitter)",
            width=270,
            options=splitter_options,
            css_classes=["filter_multiselect"],
            callback=[
                splitter_callback,
                self.y_exprs.refresh_plot_types,
                self.y_exprs.update_normalizer,
            ],
        )
        splitter_widget = self.get_ui_dict(
            type="column",
            children=params_to_dict(
                dummy=self.get_ui_dict(
                    type="row", children=params_to_dict(splitter=splitter)
                ),
                split_plots=None,
            ),
            css_classes=["splitter_widget"],
        )
        color_options = lambda data, dtypes: [""] + sorted(
            [col for col in self.dtypes if self.dtypes[col] in ["bool", "category"]]
        )  # noqa
        color_axis = self.get_ui_dict(
            type="select",
            name="Color Axis",
        ...
        if not self.initial_state:
            update_button = self.get_ui_dict(
                type="button",
                name="UPDATE PLOT",
                width=200,
                css_classes=[
        ...
        ctas = self.get_ui_dict(
            type="row",
            children={"update_button": update_button, "save_data_button": None},
            css_classes=["ctas"],
        )
        if self.data_access:
            save_data_button = self.get_ui_dict(
                type="button",
                name="DATA",
                width=50,
                css_classes=[
        ...
        plot_wrapper = self.get_ui_dict(
            type="column", children={"plot": self.plot}, css_classes=["plot_wrapper"]
        )
        if self.show_summary:
            summary_stats = self.get_ui_dict(
                type="column",
                children=params_to_dict(
                    dummy=self.get_ui_dict(
                        "### Summary Stats",
        ...
        elements = {
            "x_options": x_options,
            "splitter_widget": splitter_widget,
            "ctas": ctas,
    ...
    def _create_child_panes(self):
        self.x_pane = self.Column(
            self.Row(
                self.SectionHeader('<h3 class="section_header">X Axis: </h3>'),
                self.Spacer(width=20),
                self.x_options,
            ),
            self.Row(
                self.SectionHeader('<h3 class="section_header">Color Axis: </h3>'),
                self.color_axis,
            ),
        )
        self.splitter_pane = self.Row(
            self.SectionHeader('<h3 class="section_header">Split Plot by : </h3>'),
            self.splitter_widget,
        )
        self.y_pane = self.Row(
            self.SectionHeader('<h3 class="section_header">Y Axes: </h3>'),
            self.y_exprs.show(),
            css_classes=["y_exprs"],
        )
    ...
    def _create_standalone_widget(self):
        ...
        self.plot_config = self.Row(
            self.y_pane,
            self.x_pane,
            self.splitter_pane,
            css_classes=["plot_data_controls"],
        )
        self.viz_config = self.Column(
            self.data_processor.show(),
            self.filters.show(width=1350),
            self.plot_config,
            css_classes=["gray_bg", "full_width", "viz_config"],
        )
        # self.top_row = self.Row(self.data_changer)
        self.header = self.Row(
            self.data_loader.show(),
            "",  # For additional space at the end
        ...
        widget = self.Column(
            self.HTML(
                f"<style>{zoom_style}</style>", height=0, css_classes=["is_hidden"]
            ),
            self.header,
            self.viz_config,
            self.body,
    ...
    def _create_notebook_widget(self):
        self.controls = self.Column(
            self.data_loader.show(),
            self.data_processor.show(),
            self.filters.show(width=950),
            self.Row(
                self.y_pane,
                self.Column(self.x_pane, self.splitter_pane),
                css_classes=["plot_data_controls"],
            ),
            self.ctas,
        ...
        widget = self.Column(
            self.controls, self.body, css_classes=["iwidget", "notebook", "add_glass"]
        )
        return widget
