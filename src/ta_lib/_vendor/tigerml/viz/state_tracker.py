...
class StateTracker:
    ...
    def get_bookmarks(self):
        ...
        bookmark_links = "".join(
            [
                bookmark_link_template.format(bookmark["name"])
    ...
    def get_bookmarks_ui(self):
        ...
        self.bmk_name_save = self.Button(
            css_classes=["icon-button", "icon-ok"], width=30, height=30
        )
        self.cancel = self.Button(
            css_classes=["icon-button", "icon-cancel"], width=30, height=30
        )
        self.bookmark_edit = self.Row(
            self.bmk_name_input,
            self.bmk_name_save,
            self.cancel,
            css_classes=["content"],
        ...
        edit_widget = self.Column(
            ...
            self.bookmark_edit,
            css_classes=["edit_bookmark_widget", "overlay", "is_hidden"],
            width=0,
            height=0,
        )
        self.hidden_bmk_triggers = self.Row(
            self.bmk_select,
            self.bmk_load,
            self.bmk_name_edit,
            self.bmk_delete,
        ...
        self.import_button = self.Button(
            css_classes=["icon-button", "icon-import"], width=30, height=30
        )
        self.import_file = self.FileInput(
            accept=".json", css_classes=["is_hidden", "import_file"], width=0
        )
        self.save_button = self.Button(
            css_classes=["icon-button", "icon-save"], width=30, height=30
        )
        self.import_file.param.watch(self.import_bookmarks, "value")
        self.save_button.on_click(self.save_bookmarks)
        self.bookmarks_ops = self.Row(
            self.import_button, self.import_file, css_classes=["right"]
        )
        self.bookmarks_header = self.Row(
            self.HTML("<h3>Bookmarks</h3>", css_classes=["section_header"], width=200),
            self.bookmarks_ops,
            css_classes=["full_width"],
        )
        self.bookmarks_pane = self.Column(
            self.hidden_bmk_triggers,
            edit_widget,
            self.bookmarks_header,
            self.get_bookmarks(),
            css_classes=["bookmarks_widget"],
        )
    ...
    def update_states_list(self):
        ...
        if self.state_index != -1:
            self.all_states = self.all_states[
                : self.state_index + 1
            ]  # All next states are deleted when some changes are made by User  # noqa
            self.state_index = (
                -1
            )  # User clicked on the button manually. So a new state gets created
    ...
    def state_navigator(self):
        ...
        self.back_button = self.Button(
            css_classes=["icon-button", "icon-left"], width=50, height=40
        )
        self.forward_button = self.Button(
            css_classes=["icon-button", "icon-right"], width=50, height=40
        )
        self.bookmark_button = self.Button(
            css_classes=["icon-button", "icon-star"], width=50, height=40
        )
        self.navigator = self.Row(
            self.back_button,
            self.forward_button,
