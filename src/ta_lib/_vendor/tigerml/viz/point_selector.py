...
LASSO_SELECT_LIMIT = (
    10000  # Lasso select is inefficient, when a lot of points exists, it becomes buggy
)
...
class Selection(StatefulUI):
    ...
    def _create_ui(self):
        actions = self.togglegroup(
            options=["include", "exclude", "limit to", "compare"],
            name="Plot Actions",
            width=300,
            behavior="radio",
            css_classes=["tabs"],
    ...
    def get_selection_actions(self):
        ...
        compliment_btn = self.Button(
            css_classes=["icon-button", "icon-compliment"], width=30
        )
        download_btn = self.Button(
            css_classes=["icon-button", "icon-download"], width=30
        )
        ...
        selection_actions = self.Row(
            self.HTML('<h3 class="section_header">Actions: </h3>'),
            merge_btn,
            compliment_btn,
            preview_btn,
            download_btn,
    ...
    def save(self, event=None):
        data = pd.Series(
            [0] * len(self.parent.parent.data), index=self.parent.parent.data.index
        )
        data.name = self.name
...
class Selections(StatefulUI):
    ...
    def get_ui(self):
        ...
        self.sel_name_save = self.Button(
            css_classes=["icon-button", "icon-ok"], width=30
        )
        self.cancel = self.Button(css_classes=["icon-button", "icon-cancel"], width=30)
        self.selection_edit = self.Row(
            self.sel_name_input,
            self.sel_name_save,
            self.cancel,
            css_classes=["content"],
        )
        edit_widget = self.Column(
            self.HTML('<div class="glass"></div>'),
            self.selection_edit,
            css_classes=["edit_selection_widget", "overlay", "is_hidden"],
        )
        self.hidden_sel_triggers = self.Row(
            self.sel_select,
            self.sel_name_edit,
            self.sel_expand,
    ...
    def compute_merge_stats(self, event=None):
        ...
        self.merger_selection = [
            sel for sel in self.children if sel.name == self.merge_options.value
        ][0]
        self.sel_summary = f"Selection: {len(self.merger_selection.data)} points"
        if self.merge_logic.value == "OR":
            self.merge_result = list(
                set(self.merger_selection.data) | set(self.merging_data)
            )
        else:
            self.merge_result = list(
                set(self.merger_selection.data) & set(self.merging_data)
    ...
    def init_merging(self, sel_options=None):
        sel_options = (
            sel_options
            if sel_options is not None
            else [sel.name for sel in self.children]
        )
        self.merge_options = self.Select(
            name="Select Selection to Merge", options=sel_options
        )
    ...
    def merge_with_selection(self, event=None):
        ...
        self.merger_selection.name = (
            f"Merged Selection of {len(self.merger_selection.data)} points"
        )
        self.close_merging()
        self.refresh_selections()
    ...
    def show_data_preview_overlay(self, data):
        hide_data_btn = self.Button(
            css_classes=["icon-button", "icon-cancel"], width=50
        )
        ...
        data_preview = self.Column(
            self.HTML('<div class="glass"></div>'),
            self.Column(
                hide_data_btn,
                message,
                hvPlot(data).table(width=(len(data.columns) + 1) * 150),
                css_classes=["content", "big"],
            ),
            css_classes=["data_preview", "overlay"],
            width=800,
        )
...
class PointSelector(Selections):
    ...
    def selection_description(self):
        if self.sel_type == "xy":
            x_desc = (
                f"{self.x_selection[0]} < {self.current_x} < {self.x_selection[1]}"
                if isinstance(self.x_selection, tuple)
                else f"{self.current_x} in {self.selected_points.index.unique().tolist()}"
                if self.current_x and self.x_selection
                else ""
            )
            y_desc = (
                f"{self.y_selection[0]} < {self.current_y} < {self.y_selection[1]}"
                if isinstance(self.y_selection, tuple)
                else f"{self.current_y} in {self.selected_points[self.current_y].unique().tolist()}"
                if self.current_y and self.y_selection
                else ""
            )
    ...
    def create_selection(self, event=None):
        ...
        self._add_child(
            {"data": self.selected_data, "name": name, "description": description}
        )
        self.refresh_ui()

    def set_selection_tools(
        self, y_expr, current_plot, plot_kwargs, x_col, plotter=None
    ):
        self.reset_selection()
        ...
        def get_original_data(filtered_data):
            ...
            if x_col:
                original_filter = y_expr.source_data[filtered_data.index.name].isin(
                    filtered_data.index
                )
            if original_filter is None:
                original_filter = y_expr.source_data[y_expr.display_name].isin(
                    filtered_data[y_expr.display_name]
                )
            else:
                original_filter &= y_expr.source_data[y_expr.display_name].isin(
                    filtered_data[y_expr.display_name]
        ...
        def compute_summary_from_index(index):
            if index:
                ...
                selection_summary = pd.DataFrame(
                    {
                        "selected_points": len(self.selected_points),
                        "original_points": len(self.selected_data),
                    },
                    index=[0],
                )
            ...
            if y_selection is None and x_selection is None:
                ...
                if x_col or y_expr.plot_type.value in ["hist", "kde"]:
                    if isinstance(x_selection, tuple):
                        if y_expr.mapper_df is not None:
                            mapper_filter = (
                                x_selection[0] < y_expr.mapper_df["tigerml_mapper"]
                            ) & (y_expr.mapper_df["tigerml_mapper"] < x_selection[1])
                            x_filter = y_expr.plot_data.index.isin(
                                y_expr.mapper_df[mapper_filter][x_col]
                            )
                        else:
                            x_filter = (y_expr.plot_data.index >= x_selection[0]) & (
                                y_expr.plot_data.index <= x_selection[1]
                            )
                    else:
                ...
                if y_expr.plot_type.value not in ["hist", "kde"]:
                    if isinstance(y_selection, tuple):
                        y_filter = (
                            (y_expr.plot_data[y_expr.display_name] >= y_selection[0])
                            & (y_expr.plot_data[y_expr.display_name] <= y_selection[1])
                        ).values
                    else:
                        y_filter = (
                            y_expr.plot_data[y_expr.display_name].isin(y_selection)
                        ).values
                final_filter = None
        ...
        if (
            y_expr.segment_by
            and y_expr.plot_type.value not in ["box", "kde", "grouped_bar"]
        ) or y_expr.plot_type.value == "bar":
            data = pd.DataFrame.from_dict(
                {
                    "points": [
                        y_expr.plot_data[y_expr.display_name].min(),
                        y_expr.plot_data[y_expr.display_name].max(),
                    ]
                }
            )
            data.index = [
                y_expr.plot_data.index.values[0],
                y_expr.plot_data.index.values[-1],
            ]
            ...
            if (
                dummy_scatter.__module__ == "panel.layout"
            ):  # If the plot is split, it will be a panel layout on which
            ...
            y_expr.selection_summary = hv.DynamicMap(
                compute_summary_from_box, kdims=[], streams=[selector]
            )
            current_plot = current_plot * dummy_scatter
        else:
            if (
                y_expr.plot_type.value == "scatter"
                and not y_expr.parent.parent.splitter.value
                and "datashade" not in plot_kwargs
                and len(y_expr.plot_data) < LASSO_SELECT_LIMIT
            ):
                ...
                y_expr.selection_summary = hv.DynamicMap(
                    compute_summary_from_index, kdims=[], streams=[selector]
                )
            else:
    ...
    def selected_summary_ui(self):
        if self.y_expr:
            ...
            if self.selected_points is None:
                hide = True
            if hide:
                self.selection_summary = self.Column(
                    "<b>Selected Points</b>",
                    css_classes=["is_hidden"],
                    width=0,
                    height=0,
                ...
                self.selection_summary = self.Column(
                    "<b>Selected Points</b>", css_classes=[]
                )
            self.selection_summary.append(self.y_expr.selection_summary)
            self.add_selection_btn = self.Button(
                name="Save", css_classes=["secondary", "button"], width=100
            )
            self.add_selection_btn.on_click(self.create_selection)
            self.ctas = self.Row(self.add_selection_btn)
            self.merge_selection_btn = self.Button(
                name="Merge", css_classes=["tertiary", "button"], width=100
            )
