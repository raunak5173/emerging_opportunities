...
class DataExplorer(VizWidget, StateTracker):
    ...
    def _create_ui_elements(self):
        ...
        self.shutdown = self.Button(
            css_classes=["icon-button", "icon-shutdown", "right"], width=50, height=35
        )
    ...
    def _create_standalone_widget(self):
        ...
        self.right_bar = self.Column(
            self.get_bookmarks_ui(), css_classes=["right_side_bar"], width=440
        )
    ...
    def _create_notebook_widget(self):
        ...
        self.controls.append(
            self.Row(self.state_navigator(), bookmark_pane, css_classes=["full_width"])
        )
    ...
    def get_plot(self, **kwargs):
        ...
        limit_to_sels_data = list(
            set(
                flatten_list(
                    [
                        sel.data
                        for sel in self.selector.children
                        if sel.actions.value == "limit to"
                    ]
                )
            )
        )  # noqa
        exclude_sels_data = list(
            set(
                flatten_list(
                    [
                        sel.data
                        for sel in self.selector.children
                        if sel.actions.value == "exclude"
                    ]
                )
            )
        )  # noqa
        ...
        # super().get_plot()
        compare_sels = [
            sel for sel in self.selector.children if sel.actions.value == "compare"
        ]
        ...
        if compare_sels:
            for sel in compare_sels:
                ...
                for y_expr in self.y_exprs.children:
                    y_expr.plot_data.rename(
                        columns={
                            y_expr.display_name: y_expr.display_name + f" - {sel.name}"
                        },
                        inplace=True,
                    )
                self.sort_data()
                if comparison_plots:
                    ...
                    comparison_plots = self.create_plot()
            compare_sels_data = list(
                set(flatten_list([sel.data for sel in compare_sels]))
            )
