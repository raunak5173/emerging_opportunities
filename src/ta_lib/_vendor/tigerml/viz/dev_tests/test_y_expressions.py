...
@pytest.mark.parametrize("col_name", ["int_col", "float_col"])
@pytest.mark.parametrize("color_axis", ["", "bool_col", "date_col", "string_col"])
@pytest.mark.parametrize(
    "plot_type", ["kde", "hist", "box", "violin", "table"]
)  # ['kde', 'hist', 'box', 'violin']
@pytest.mark.parametrize("test_type", ["t1", "t2", "t3"])  # 't4' has to be debugged
...
@pytest.mark.parametrize("col_name", ["date_col", "bool_col", "string_col"])
@pytest.mark.parametrize("color_axis", ["", "bool_col", "date_col", "string_col"])
@pytest.mark.parametrize("plot_type", ["bar", "table"])
@pytest.mark.parametrize("test_type", ["t1", "t2", "t4"])  # 't3' has to debugged
@settings(max_examples=10, deadline=None)
@given(test_df=mixed_df(), agg_func=data())
...
@pytest.mark.parametrize(
    "y_col", ["float_col", "date_col", "int_col", "string_col"]
)  # 'bool_col' fails in testing but in UI it fails only during StateTracking
...
def test_univariate_plot(test_df, y_col, color_axis, segment_by, agg_f, plot_type):
    ...
    explorer.color_axis.value = color_axis.draw(
        sampled_from(explorer.color_axis.values)
    )
    segment_by_val = [True, False]
    explorer.y_exprs.children[0].have_color_axis.value = segment_by.draw(
        sampled_from(segment_by_val)
    )
    ...
    explorer.y_exprs.children[0].plot_type.value = plot_type.draw(
        sampled_from(
            [
                x
                for x in explorer.y_exprs.children[0].plot_type.values
                if x not in ["table"]
            ]
        )
    )
...
@pytest.mark.parametrize(
    "y_col", ["float_col", "date_col", "bool_col", "int_col", "string_col"]
)
@pytest.mark.parametrize(
    "x_col", ["", "float_col", "date_col", "bool_col", "int_col", "string_col"]
)
@pytest.mark.parametrize("order", ["with order", "with out order"])
@settings(max_examples=50, deadline=None)
...
def test_bivariate_plot(y_col, x_col, order, color_axis, segment_by, agg_f, plot_type):
    ...
    if (
        x_col != y_col
        and not (y_col in ["string_col", "date_col"] and x_col == "")
        and xy_list != str_bool_list
    ):
        explorer.y_exprs.children[0].col_name.value = y_col
...
@pytest.mark.parametrize("grid_split", [True, False])
...
def test_bivariate_plot_with_splitter(
    y_col, x_col, splitter, color_axis, segment_by, agg_f, plot_type, grid_split
):
    file_path = os.path.join(HERE, "t1.csv")
    test_df = td.read_csv(file_path)
    test_df = test_df.convert_datetimes()
    ...
    if x_col != y_col and not (y_col in ["string_col", "date_col"] and x_col == ""):
        ...
        explorer.splitter.value = splitter.draw(
            lists(elements=sampled_from(splitter_vals), min_size=0, unique=True)
        )
        ...
        if not (x_col == "float_col" and y_col in ["int_col", "date_col"]) and not (
            ...
            color_axis_vals = [
                x
                for x in explorer.color_axis.values
                if x not in explorer.splitter.value + [x_col]
            ]
...
@pytest.mark.parametrize("y_col1", ["float_col", "cat_col"])
@pytest.mark.parametrize("x_col", ["cat_col2"])
@pytest.mark.parametrize("y_col2", ["float_col3", "cat_col3"])
@pytest.mark.parametrize("color_col", ["cat_col4"])
@pytest.mark.parametrize("t_type", ["render", "non_render"])
@settings(max_examples=50, deadline=None)
@given(
    segment_by1=data(),
    agg_f1=data(),
...
def test_multiple_yexpr(
    ...
    if x_col != y_col1 and x_col != y_col2:
        ...
        explorer.y_exprs.children[0].have_color_axis.value = segment_by1.draw(
            sampled_from(segment_by1_val)
        )
        explorer.y_exprs.children[0].agg_func.value = agg_f1.draw(
            sampled_from(explorer.y_exprs.children[0].agg_func.values)
        )
        explorer.y_exprs.children[0].plot_type.value = plot_type1.draw(
            sampled_from(explorer.y_exprs.children[0].plot_type.values)
        )
        explorer.y_exprs.add_new_y()
        explorer.y_exprs.children[1].col_name.value = y_col2
        segment_by2_val = [True, False]
        explorer.y_exprs.children[1].have_color_axis.value = segment_by2.draw(
            sampled_from(segment_by2_val)
        )
        explorer.y_exprs.children[1].agg_func.value = agg_f2.draw(
            sampled_from(explorer.y_exprs.children[1].agg_func.values)
        )
        explorer.y_exprs.children[1].plot_type.value = plot_type2.draw(
            sampled_from(explorer.y_exprs.children[1].plot_type.values)
        )
