...
def extract_hv_plot_data(plot, n_rows=100):
    ...
    if isinstance(plot.data, pd.DataFrame):
        if plot.__class__.__name__ == "Distribution":
            ...
            req_cols = []
            if (
                plot._kdims_param_value
                and plot._kdims_param_value[0]._name_param_value in plot.data.columns
            ):
                req_cols += [plot._kdims_param_value[0]._name_param_value]
            if (
                plot._vdims_param_value
                and plot._vdims_param_value[0]._name_param_value in plot.data.columns
            ):
