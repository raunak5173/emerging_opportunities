...
Use regression to identify sale drivers for our vendor A
...
# Notebooks
    ...
    Run sequentially
    - data_discovery.ipynb - EDA
    - data processing.ipynb (Pass the theme name & theme Id) for the data to be processed and generated
    - model_experimentation.ipynb (Model Building on the data processed)

# Production
* Change directory to production folder
    Run command sequentially :-
    - python cli.py job run --job-id data-cleaning
    - python cli.py job run --job-id feat-engg
    - python cli.py job run --job-id model-gen
    - python cli.py job run --job-id model-eval
